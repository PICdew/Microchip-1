...
unsigned char OERRcounter;
unsigned char FERRcounter;

/* UART routines */
void UART_Init(UART_BAUD baudselect, UART_CONFIG conf, UART_MODE mode, unsigned char nine)
{
    /*Clear error counts */
    OERRcounter = 0;    
    FERRcounter = 0;
    ...
    UARTDTPIN = 1; /* as prescribed in datasheet. */
    RCSTAbits.RX9 = (nine!=0) ? 1 : 0;
    RCSTAbits.ADDEN = 0;
    TXSTAbits.TX9 = (nine!=0) ? 1 : 0;
    
    switch (baudselect)
    {
        case UART_BAUD_MAXIMUM:
...
unsigned char UART_Read(unsigned char *buffer)
{ 
